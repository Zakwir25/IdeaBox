                            ...
                            @foreach ($approvers->where('section_id', $section->id) as $approver)
                                <div class="approver-card card mb-2">
                                    <div class="card-body d-flex justify-content-between align-items-center">
                                        <div>
                                            <p><strong>NIK:</strong>
                                                {{ isset($approver->user->nik) ? $approver->user->nik . ' - ' . $approver->user->name : '-' }}</p>
                                            <p><strong>Level:</strong> {{ isset($approver->level) ? $approver->level : '-' }}</p>
                                            <p><strong>Is Special:</strong> {{ isset($approver->is_special) ? ($approver->is_special ? 'Yes' : 'No') : '-' }}
                                            </p>
                                            <p><strong>Skip To Level:</strong> {{ isset($approver->skip_to_level) ? $approver->skip_to_level : '-' }}</p>
                                        </div>
